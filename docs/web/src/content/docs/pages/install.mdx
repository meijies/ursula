---
title: Install
description: Build Ursula from source. Ships two binaries — the `ursula` server and the `ursulactl` operator CLI.
---
...
Both should print supported CLI flags. Then run a short smoke server (defaults to `127.0.0.1:4437`, in-memory engine, core count from your CPU):
