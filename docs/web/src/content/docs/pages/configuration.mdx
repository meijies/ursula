...
| `--wal-dir DIR` | none | Per-group JSON log + per-core `journal.bin` for single-node persistence |
...
| Variable | Purpose |
| -------- | ------- |
| `URSULA_TOKIO_CONSOLE` | When set and the binary was built with the `tokio-console` feature, initializes the console subscriber on startup |
| `RUST_LOG` | Standard tracing filter `ursula=info,ursula_runtime=info,ursula_raft=info` is a reasonable baseline |
