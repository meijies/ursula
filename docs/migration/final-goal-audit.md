...
| Minimal HTTP adapter | `crates/ursula` | Axum adapter routes PUT/POST/GET/HEAD requests to `ShardRuntime` without owning stream state | Implemented prototype |
...
| Raw HTTP ingress diagnostic | `ursula-raw` | Diagnostic HTTP/1 server implements only the `perf_compare` create and append-batch subset over the same `ShardRuntime`, bypassing axum/hyper routing and response rendering for ingress ceiling checks | Diagnostic implemented |
| `perf_compare` write/small smoke | `cargo run -p perf-compare --bin perf_compare -- --targets ursula --phases write,small ...` | Existing `perf_compare` Ursula target drove `ursula` write and small-event batch append phases with zero errors | Smoke tested |
| Release `perf_compare` write/small smoke | `cargo run --release -p perf-compare --bin perf_compare -- --targets ursula --phases write,small --concurrency 256 --throughput-secs 5 ...` | Release `ursula` with 10 cores and 160 groups completed write/small batch phases with zero errors; server metrics reported 10 active cores and 160 active groups | Smoke tested |
| CPU-sampled release `perf_compare` | release `ursula` plus release `perf_compare` write/small runs at concurrency 256, 1024, and 4096 | Throughput reached up to 1.13M logical appends/s and all configured cores/groups were active, but server CPU still stayed far below 10 cores | Not met |
| EC2 CPU-saturation check | `c7gn.8xlarge` client, three `c7g.4xlarge` servers, release `ursula --raft-memory`, release `perf_compare`, `ursula-raft-runtime-stress`, `ursula-raw`, `perf record` | Direct OpenRaft runtime stress on one `c7g.4xlarge` reached 1,598% CPU, proving the host/runtime can saturate 16 vCPUs; one `perf_compare` process remained around 2.5-3.1 server cores per target, three processes against one server pushed it to 746.7% average active CPU and 967.0% peak with no group-mailbox backlog, and five processes reached 1,513.0% peak but with status-0 errors and mailbox-full events; raw HTTP kept the same single-process throughput class; client `perf` showed per-request reqwest/HTTP allocation, URL/header construction, and scheduling as the first limiter | Not met |
...
| `perf_compare` read smoke | `cargo run -p perf-compare --bin perf_compare -- --targets ursula --phases read ...` | Existing `perf_compare` Ursula target drove catch-up reads against `ursula` with zero errors | Smoke tested |
...
- `cargo test --workspace --all-targets`: passed after the shared
  ...
  covered 128 unit/integration tests across `ursula`, `ursula-raft`,
...
- Static Raft cluster config parser tests:
  `cargo test -p ursula --bin ursula raft_cluster_config --
  --nocapture` and `cargo test -p ursula --bin ursula
  rejects_conflicting_raft_node_id_from_config_file -- --nocapture` passed.
- Static gRPC durable-log wiring tests:
  `cargo test -p ursula static_grpc_raft_runtime_can_use_core_journal
  -- --nocapture`, `cargo test -p ursula
  static_grpc_raft_runtime_recovers_from_core_journal_after_restart --
  --nocapture`, `cargo test -p ursula
  ...
  `cargo test -p ursula
  static_grpc_raft_installs_snapshot_for_late_learner_with_core_journals --
  --nocapture`, `cargo test -p ursula --test static_cluster_cli
  ...
  AWS_DEFAULT_REGION=us-east-1 cargo test -p ursula --test
  static_cluster_cli
  cli_static_grpc_raft_log_dir_recovers_replicated_s3_cold_manifest_after_restart
  ...
  `cargo test -p ursula --bin ursula
...
- Official Durable Streams conformance against `ursula --raft-memory`:
  `300 passed / 300` after the cold write-admission changes.
- Official Durable Streams conformance against `ursula --raft-memory` with
...
- Current-code official Durable Streams conformance against durable OpenRaft
  ...
  `ursula --raft-log-dir /tmp/ursula-conformance-raft-log-noflush`,
...
- Current-code official Durable Streams conformance after adding the narrow
  ...
  `ursula --raft-log-dir /tmp/ursula-current-conformance-raft-log`,
  `URSULA_COLD_BACKEND=memory`, `URSULA_COLD_FLUSH_INTERVAL_MS=1`,
...
- TCP smoke test against `ursula`: `PUT` returned `201`, `POST` returned
  `204`, and `GET ?offset=2&max_bytes=3` returned `cde`.
- TCP append-batch smoke test against `ursula`: `POST /append-batch`
  returned `[{"status":204},{"status":204}]`, and readback returned `abcde`.
- TCP metrics smoke test against `ursula`: after two batch appends,
...
- TCP WAL recovery smoke test against `ursula --wal-dir`: create returned
...
- Runtime group dispatch test: while one group engine is blocked inside create,
  ...
  command at a time.
- TCP long-poll smoke test against `ursula`: timeout returned `204` with
  ...
  after both paths.
- TCP stream-seq smoke test against `ursula`: append with `stream-seq:
...
- HTTP producer-header router test: duplicate `Producer-Id`/`Producer-Epoch`/
  ...
  bytes.
- TCP delete smoke test against `ursula`: create returned `201`, append
  returned `204`, delete returned `204`, subsequent HEAD returned `404`, and
  append-after-delete returned `404 StreamNotFound`.
- Local `perf_compare` smoke against `ursula` with `--core-count 4` and
...
- Release local `perf_compare` write/small smoke against `ursula` with
...
- CPU-sampled release `perf_compare` write/small run against `ursula` with
  `--core-count 10`, `--raft-group-count 160`, `--concurrency 256`,
...
- A minimal-ack control run added `Prefer: return=minimal` for Ursula
  ...
  conclusion.
- A raw HTTP/1 ingress control used `ursula-raw` with the same
...
- WAL-backed release `perf_compare` smoke after adding the group-level
  append-batch WAL path: `ursula --wal-dir` with `--core-count 10` and
...
- OpenRaft-backed release `perf_compare` smoke after adding `--raft-log-dir`
  and file-log metrics: `ursula --raft-log-dir` with `--core-count 10`
...
- OpenRaft in-memory release `perf_compare` smoke after adding
  `ursula --raft-memory`: release `ursula` with `--core-count 4` and
...
- OpenRaft in-memory release `perf_compare` control after changing
  ...
  `ursula --raft-memory` with `--core-count 10`, `--raft-group-count 160`,
...
- Tokio-console attribution for the same OpenRaft in-memory shape: rebuilding
  release `ursula` with `--features tokio-console` and
...
- OpenRaft write-stage metrics were added for `raft_write_many_*` and
  ...
  `cargo test -p ursula-raft -p ursula-runtime -p ursula` passed. A
...
- A 5-second macOS `sample` during a 15-second release HTTP OpenRaft in-memory
  ...
  cause.
- Local `perf_compare` read smoke against `ursula`: read phase completed
  ...
  empty mailboxes.
- Local `perf_compare` SSE smoke against `ursula`: SSE phase delivered 20
  events to 2 readers with zero errors.
- Local `perf_compare` mixed smoke against `ursula`: mixed phase completed
...
- Local `perf_compare` latency smoke against `ursula`: 20 append probes and
...
- Production multi-node Raft transport and membership management. Current
  ...
  gate open.
  `cargo test -p ursula --all-targets` passed for lib/bin tests; one CLI
