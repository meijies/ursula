//! Compile-time-safe shape assertions for CI-generated minimize artifacts.
//!
...
//! `crates/ursula-sim/src/madsim_harness.rs` therefore breaks the shape fn at
            ...
            other => {
                return Err(format!(
                    "unknown SimFaultAction variant name {other:?} in --steps-exact; \
                     update parse_variant_predicate in ursula-sim-assert-shape.rs"
                )
                .into());
            }
        };
